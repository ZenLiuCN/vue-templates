...
export interface MomentLib {
  (val: string | String, format?: string): Moment
  (year: number, month: number, day: number): Moment
  (): Moment
  duration(count: number, unit: string): Duration
  duration(parse?: object | string | number): Duration
  locale(lang: string): void
  unix(number: number): Moment
  now(): Moment
}
let vue_moment = function(Vue, options) {
  ...
  Vue.filter(
    'timestamp',
    (m: number, format: string = 'YYYY-MM-DD HH:mm:ss') => {
