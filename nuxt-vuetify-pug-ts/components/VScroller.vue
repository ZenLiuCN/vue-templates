...
@mousedown='mouseDown($event)', @mousemove='mouseMove($event)', @mouseup='mouseUp($event)')
	._v-content(:id='contentId')
		.pull-to-refresh-layer(v-if='onRefresh', :class="{'active': state == 1, 'active refreshing': state == 2}")
			span.spinner-holder
				v-icon(:color='refreshLayerColor', v-if='state != 2') {{state===0?'mdi-arrow-up':'mdi-arrow-down'}}
				span.text(v-if='state != 2', :style='{color: refreshLayerColor}', v-text='refreshText')
				span(v-if='state == 2')
					slot(name='refresh-spinner')
						v-progress-circular(indeterminate :style='{fill: refreshLayerColor, stroke: refreshLayerColor}')
		slot
		.loading-layer(v-if='showInfiniteLayer')
			span.spinner-holder(:class="{'active': showLoading}")
				slot(name='infinite-spinner')
					v-progress-circular(indeterminate :style='{fill: loadingLayerColor, stroke: loadingLayerColor}')
			.no-data-text(:class="{'active': !showLoading && loadingState == 2}", :style='{color: loadingLayerColor}', v-text='noDataText')
</template>
...
import { Component, Vue, Mutation, Prop } from 'nuxt-property-decorator'
import { AdvScroller, Options } from '~/components/Scroller.ts'
...
@Component({ name: 'VScroller' })
export default class VScroller extends Vue {
  @Prop() onRefresh!: Function
  @Prop() onInfinite!: Function
  @Prop({ type: String, default: '下拉刷新' }) refreshText!: string
  @Prop({ type: String, default: '没有更多数据' }) noDataText!: string
  @Prop({ type: String, default: '100%', validator: widthAndHeightValidator }) width!: string
  @Prop({ type: String, default: '100%', validator: widthAndHeightValidator }) height!: string
  @Prop({ type: Boolean, default: false }) snapping!: boolean
  @Prop({ type: Number, default: 100 }) snapWidth!: number
  @Prop({ type: Number, default: 100 }) snapHeight!: number
  @Prop({ type: Number, default: 250 }) animationDuration!: number
  @Prop({ type: Boolean, default: true }) animating!: boolean
  @Prop({ type: Boolean, default: true }) bouncing!: boolean
  @Prop({ type: String, default: '#AAA' }) refreshLayerColor!: string
  @Prop({ type: String, default: '#AAA' }) loadingLayerColor!: string
  @Prop({ type: String, }) cssClass!: string
  @Prop({ type: Number, default: 0 }) minContentHeight!: number

  containerId: string = 'outer-' + Math.random().toString(36).substring(3, 8)
  contentId: string = 'inner-' + Math.random().toString(36).substring(3, 8)
  state: number = 0 // 0: pull to refresh, 1: release to refresh, 2: refreshing
  loadingState: number = 0 // 0: stop, 1: loading, 2: stopping loading
  showLoading: boolean = false
  container!: HTMLElement
  content!: HTMLElement
  scroller!: AdvScroller
  pullToRefreshLayer!: HTMLElement
  mousedown: boolean = false
  infiniteTimer: any = undefined
  resizeTimer: any = undefined

  get w() { return widthAndHeightCoerce(this.width) }
  get h() { return widthAndHeightCoerce(this.height) }
  get showInfiniteLayer() {
    let contentHeight = 0
    this.content
  ...
  mounted() {
    ...
    this.scroller = new AdvScroller(render, {
      scrollingX: false,
